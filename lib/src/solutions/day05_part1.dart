...
int solve(List<String> input) {
  ...
  return mapMultipleMultipleTimes(seeds, mss).min();
}

Iterable<int> mapMultipleMultipleTimes(
  Iterable<int> ns,
  Iterable<Iterable<List<int>>> mss,
) {
  for (Iterable<List<int>> ms in mss) {
    ns = mapMultiple(ns, ms);
  }
  return ns;
}

Iterable<int> mapMultiple(
  Iterable<int> ns,
  Iterable<List<int>> ms,
) {
  return ns.map((n) => map(n, ms));
}
