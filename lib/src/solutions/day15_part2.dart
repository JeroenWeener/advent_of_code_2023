...
int solve(String input) {
  List<List<String>> lensBoxes = List.generate(256, (_) => []);

  for (String instruction in input.split(',')) {
    final (hash, label, focalLength) = parseInstruction(instruction);
    if (focalLength == null) {
      lensBoxes[hash].removeWhere((l) => l.startsWith(label));
    } else {
      int index = lensBoxes[hash].indexWhere((l) => l.startsWith(label));
      if (index != -1) {
        lensBoxes[hash][index] = instruction;
      } else {
        lensBoxes[hash].add(instruction);
      }
    }
  }
  return lensBoxes.indexed.map((e) {
    final (boxIndex, box) = e;
    return box.indexed
        .map((e) => (e.$1 + 1) * int.parse(e.$2.split('=').second))
        .map((e) => e * (boxIndex + 1))
        .sum();
  }).sum();
}

(int, String, int?) parseInstruction(String instruction) {
  String label = instruction.split(RegExp(r'[=-]')).first;
  int? focalLength = int.tryParse(instruction.last);
  int hash = label.codeUnits.fold(0, (a, b) => (a + b) * 17 % 256);
  return (hash, label, focalLength);
}
