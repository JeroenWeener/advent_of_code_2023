...
int solve(List<String> input) {
  return input
      .skip(2)
      .map((e) => e.split(' ').first)
      .where((node) => node.last == 'A')
      .map(
    (location) {
      int step = 0;
      while (location.last != 'Z') {
        int direction = input.first[step % input.first.length] == 'L' ? 0 : 1;
        location = input
            .skip(2)
            .firstWhere((e) => e.startsWith(location))
            .regexAllMatches(r'\w{3}')
            .skip(1)
            .elementAt(direction);
        step++;
      }
