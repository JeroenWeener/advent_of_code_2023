...
void main() async {
  ...
  assert(solve(testInput) == 30);

  int answer = solve(puzzleInput);
  print(answer);
}

Map<String, Iterable<String>> cardMappings = {};
RegExp reNumber = RegExp(r'\d+');

int solve(List<String> input) {
  for (String card in input) {
    final [a, b] = card.split('|');
    final cardNumber = int.parse(reNumber.firstMatch(a)!.group(0)!);
    final winningNumbers =
        reNumber.allMatches(a).skip(1).map((e) => e.group(0)!).map(int.parse);
    final ns = reNumber.allMatches(b).map((e) => e.group(0)!).map(int.parse);

    int matches = ns.where((n) => winningNumbers.contains(n)).length;
    cardMappings[card] = input.skip(cardNumber).take(matches);
  }

  int lineNumber = 0;
  while (lineNumber < input.length) {
    input.addAll(cardMappings[input[lineNumber]]!);
    lineNumber++;
  }
