...
List<String> order = 'A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, 2'.split(', ');
...
bool hasFiveOfAKind(String hand) {
  int c1 = hand.codeUnits.first;
  return hand.codeUnits.every((c) => c == c1);
}

bool hasFourOfAKind(String hand) {
  final [c1, c2, _, _, _] = hand.codeUnits;
  return hand.codeUnits.where((c) => c == c1).length == 4 ||
      hand.codeUnits.where((c) => c == c2).length == 4;
}

bool hasFullHouse(String hand) {
  return hand.codeUnits.toSet().length == 2;
}

bool hasThreeOfAKind(String hand) {
  final [c1, c2, c3, _, _] = hand.codeUnits;
  return hand.codeUnits.where((c) => c == c1).length == 3 ||
      hand.codeUnits.where((c) => c == c2).length == 3 ||
      hand.codeUnits.where((c) => c == c3).length == 3;
}
