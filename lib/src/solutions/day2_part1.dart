...
int solve(List<String> input) {
  return input.indexed
      .map((pair) => isValidGame(pair.$2) ? pair.$1 + 1 : 0)
      .sum();
}

bool isValidGame(String game) {
  return game.split(': ').second.split('; ').every((set) => isValidSet(set));
}

bool isValidSet(String set) {
  return getCubesFromSet(set)
      .entries
      .every((entry) => entry.value <= bag[entry.key]!);
}
