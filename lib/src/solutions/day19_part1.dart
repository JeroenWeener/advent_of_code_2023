...
int solve(String input) {
  Iterable<Workflow> workflows = input //
      .split('\n\n')
      .first
      .split('\n')
      .map(Workflow.parse);
  ...
  return input
      ...
      .where((machinePart) {
        Statement currentStatement =
            workflows.firstWhere((w) => w.name == 'in').condition;

        while (true) {
          if (currentStatement is AcceptingStatement) {
            return true;
          }
          if (currentStatement is RejectingStatement) {
            return false;
          }
          if (currentStatement is Condition) {
            currentStatement = currentStatement.compute(machinePart);
          }
          if (currentStatement is MovingStatement) {
            String name = currentStatement.name;
            currentStatement =
                workflows.firstWhere((w) => w.name == name).condition;
          }
        }
      })
      .map((p) => p.total)
...
class MachinePart {
  ...
  factory MachinePart.parse(String description) {
    final [x, m, a, s] = description.extractInts();
...
class Condition implements Statement {
  ...
  factory Condition.parse(String instruction) {
    ...
    return Condition((MachinePart part) {
      if (operator == '<') {
        return part.get(property) < value ? a : b;
      } else {
        return part.get(property) > value ? a : b;
      }
    });
  ...
  final Statement Function(MachinePart) compute;
}
